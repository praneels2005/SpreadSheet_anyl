...
- `Test.xlslx` - the final file with static values that must be uploaded to the dashboard
...
7. **Using Cached Results from Excel**

The dashboard cannot read dynamic values generated by formulas in the Excel sheet. To ensure the data is accessible, you must convert the formula outputs to static values.

1. Open the Excel file.
2. Copy the cells containing the computed results.
3. Paste them back using **Paste Special → Values** to replace the formulas with their computed results.
4. Save the file.

This converts the dynamic outputs into static values that the dashboard can read.
